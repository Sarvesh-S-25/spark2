...
```powershell
node .claude\scripts\gemini.mjs --dirs server/splitter "what do the three passes do?"
```
...
```
[context]   6 files · 49KB from server/splitter     what was collected
...
node .claude\scripts\gemini.mjs --dirs server/splitter --dry-context   # collector only
